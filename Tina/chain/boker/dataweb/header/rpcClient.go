...
func (c *Client) GetTx(hash common.Hash) (number *big.Int, err error) {

	if nil == c.EthClient {
		return big.NewInt(0), errors.New("interface not specified, call AtInterface first!")
	}

	_, _, err = c.EthClient.TransactionByHash(context.Background(), hash)
	if err != nil {
...
func (c *Client) SetPicFromData(data []byte) (hash common.Hash, err error) {
	...
	if len(data) > int(protocol.MaxDataSize) {
		return common.Hash{}, errors.New("SetPicFromData length too more than MaxDataSize(5MB)")
	}
...
func (c *Client) SetData(data []byte) (hash common.Hash, err error) {
	...
	if len(data) >= protocol.MaxDataSize {
		return common.Hash{}, errors.New("SetData length too more than MaxDataSize(5MB)")
	}
